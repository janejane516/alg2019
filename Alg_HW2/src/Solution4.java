...
class Solution4 {
	...
	//time complexity: theta(n) -> 가능한 패턴이 6개이므로, 총 6(n-1)번 반복되고, 여기에 constant factor가 더해지기 떄문에.
	public static void solve() {
	    ...
	    for(int i=1; i<n; i++) {
	        peb[i][0] = Math.max(peb[i-1][3], peb[i-1][4]) + (A[0][i] - A[2][i]);
	        peb[i][1] = Math.max(peb[i-1][2], peb[i-1][5]) + (A[0][i] - A[1][i]);
            peb[i][2] = Math.max(peb[i-1][1], peb[i-1][5]) + (A[1][i] - A[2][i]);
            peb[i][3] = Math.max(peb[i-1][0], peb[i-1][4]) + (A[2][i] - A[1][i]);
            peb[i][4] = Math.max(peb[i-1][0], peb[i-1][3]) + (A[1][i] - A[0][i]);
            peb[i][5] = Math.max(peb[i-1][1], peb[i-1][2]) + (A[2][i] - A[0][i]);
        }
