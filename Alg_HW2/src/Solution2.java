...
class Solution2 {
	...
	*/
    ...
    //time complexity: theta(n^3) -> 삼중 for-loop을 돌기 때문에.
	public static void solve() {
	    m = new Element[n+1][n+1];
	    for(int i=1; i<=n; i++) {
	        m[i][i] = new Element();
	        m[i][i].setFirst(s.charAt(i-1));
        }
        ...
        for(int r=1; r<=n; r++) {
            ...
            for(int i=1; i<=n-r; i++) {
                int j = i + r;
                m[i][j] = new Element();
                //for-loop setElement r번
                for(int k=i; k<=j-1; k++) {
                    m[i][j].setElement(m[i][k], m[k+1][j]);
                }
            }
        }
    ...
    public static class Element {
        ...
        public void setElement(Element X, Element Y) {
	        a += (X.a * Y.c + X.b * Y.c + X.c * Y.a);
	        b += (X.a * Y.a + X.a * Y.b + X.b * Y.b);
	        c += (X.b * Y.a + X.c * Y.b + X.c * Y.c);
        }
    }
