...
class Solution1 {
    ...
    //time complexity: theta(E*N) -> due to the double for-loop
    public static void BellmanFord1(int src) {
        for(int i=1; i<=N; i++) {
        ...
        for(int i=1; i<N; i++) {
            for(int j=0; j<E; j++) {
                u = U[j];
                v = V[j];
                if((d[u] + W[j] < d[v]) && d[u] != Integer.MAX_VALUE) {
                    d[v] = d[u] + W[j];
                }
            }
        }
    ...
    //time complexity: O(E*N) -> since the maximum j*k loop cycling number is E.
    public static void BellmanFord2(int src) {
        ...
        for(int i=1; i<N; i++) {
            for(int j=0; j<len; j++) {
                u = L[j];
                for(int k=0; k<edge_num[u]; k++) {
                    v = V[R[u][k]];
                    w = W[R[u][k]];
                    if((d[u] + w < d[v]) && d[u] != Integer.MAX_VALUE) {
                        d[v] = d[u] + w;
                        check[v] = true;
                    }
                }
